...
function validateDonation() {
    ...
    const creditCardDetails = document.getElementById("credit-card-details").value.trim();

    // Clear previous error messages
    document.querySelectorAll('.donation-error').forEach(e => e.remove());

    // Validate donation amount
    if (!donationAmountElement.value || isNaN(donationAmountElement.value) || parseInt(donationAmountElement.value) <= 0) {
        showError(donationAmountElement, 'Please select a valid donation amount');
        isValid = false;
    }

    // Validate donor's name
    if (!donorName) {
        showError(document.getElementById("donor-name"), 'Please enter your name');
        isValid = false;
    }

    // Validate donor's address
    if (!donorAddress) {
    ...
    if (!creditCardDetails || creditCardDetails.length < 16) {
        showError(document.getElementById("credit-card-details"), 'Please enter valid credit card details');
        isValid = false;
    }
